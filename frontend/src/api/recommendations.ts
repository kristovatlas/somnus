...
export function createExperiment(data: ExperimentCreate): Promise<ExperimentOut> {
  return fetchJson<ExperimentOut>('/api/experiments', {
    method: 'POST',
...
export function updateExperiment(id: number, data: ExperimentUpdate): Promise<ExperimentOut> {
  return fetchJson<ExperimentOut>(`/api/experiments/${id}`, {
    method: 'PATCH',
    body: JSON.stringify(data),
  })
