...
describe('syncOura', () => {
  ...
  it('passes date params as query string', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ synced_count: 0, start_date: '2026-02-01', end_date: '2026-02-02', errors: [] })),
    )
  ...
  it('throws on error response', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ detail: 'Token not configured' }), { status: 403 }),
    )
