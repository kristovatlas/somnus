...
const fullDashboard: DashboardData = {
  ...
  trends: [
    { date: '2025-06-19', sleep_score: 80, avg_hrv: 40, deep_minutes: 70, rem_minutes: 90 },
    { date: '2025-06-20', sleep_score: 85, avg_hrv: 42, deep_minutes: 75, rem_minutes: 95 },
    { date: '2025-06-21', sleep_score: 87, avg_hrv: 45, deep_minutes: 80, rem_minutes: 100 },
  ],
  stage_averages: {
    avg_deep_minutes: 75,
    avg_rem_minutes: 95,
...
describe('DashboardPage', () => {
  ...
  it('shows sessions recommended when not meeting minimum', async () => {
    mockFetchWith({
      ...fullDashboard,
      red_light_summary: { ...fullDashboard.red_light_summary, session_count: 1, meets_minimum: false },
    })
    renderPage()
    await waitFor(() => {
      expect(screen.getByText(/2 more sessions recommended/)).toBeInTheDocument()
    })
  })
  ...
  it('shows empty consistency message', async () => {
    ...
    renderPage()
    await waitFor(() => {
      expect(screen.getByText(/Need more data for consistency/)).toBeInTheDocument()
    })
  })
