    ...
    <div className="report-card" data-testid="top-factors">
      ...
      {positive && (
        <p className="report-factor-line">
          Associated with better sleep score:{' '}
          <strong>{positive.label}</strong>{' '}
          <span className="report-factor-r">(r={positive.pearson_r.toFixed(3)})</span>
        </p>
      )}
      {negative && (
        <p className="report-factor-line">
          Associated with worse sleep score:{' '}
          <strong>{negative.label}</strong>{' '}
          <span className="report-factor-r">(r={negative.pearson_r.toFixed(3)})</span>
        </p>
      )}
    </div>
  )
