...
export function WeeklyReportView({ year, week, onNavigate }: Props) {
  ...
  if (loading) return <div className="report-empty">Loading weekly report...</div>
    ...
    <div data-testid="weekly-report-view">
      ...
      {data.has_insufficient_data && (
        <div className="report-insufficient" data-testid="insufficient-data">
          Not enough data for a full report. Need at least 2 nights of sleep data this week.
        </div>
      )}
      ...
      {data.consistency && (
        <div className="report-card" data-testid="consistency-card">
          ...
          <div className="report-consistency-pills">
            <span className="report-pill">{'\u03C3'} {data.consistency.sigma_minutes.toFixed(0)} min</span>
            {data.consistency.delta_minutes !== null && (
              <span className="report-pill">{'\u03B4'} {data.consistency.delta_minutes.toFixed(0)} min</span>
            )}
            {data.consistency.weekend_drift_minutes !== null && (
              <span className="report-pill">{'\u0394'} {data.consistency.weekend_drift_minutes.toFixed(0)} min</span>
            )}
          </div>
        </div>
      )}

      <TopFactorsCard positive={data.top_positive_factor} negative={data.top_negative_factor} />

      <div className="report-export">
        <a href={weeklyExportUrl(year, week)} target="_blank" rel="noopener noreferrer" className="report-export-link">
          Export as HTML
        </a>
      </div>
    </div>
