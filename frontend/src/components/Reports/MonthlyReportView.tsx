...
export function MonthlyReportView({ year, month, onNavigate }: Props) {
  ...
  if (loading)
    return <div className="report-empty">Loading monthly report...</div>;
    ...
    <div data-testid="monthly-report-view">
      ...
      {data.has_insufficient_data && (
        <div className="report-insufficient" data-testid="insufficient-data">
          Not enough data for a full report. Need at least 4 nights of sleep
          data this month.
        </div>
      )}
      ...
      {data.active_experiment && (
        <div className="report-card" data-testid="experiment-section">
          ...
          <p>
            <strong>{data.active_experiment.factor_label}</strong>:{" "}
            {data.active_experiment.hypothesis}
          </p>
          <p className="report-muted">
            {data.active_experiment.start_date} to{" "}
            {data.active_experiment.end_date}
            {" \u2014 "}
          ...
          {weekliesOpen && (
            <div className="report-weekly-list" data-testid="weekly-summaries">
              {data.weekly_summaries.map((ws) => (
                <div
                  key={`${ws.iso_year}-${ws.iso_week}`}
                  className="report-weekly-item"
                >
      ...
      <div className="report-export">
        <a
          href={monthlyExportUrl(year, month)}
          target="_blank"
