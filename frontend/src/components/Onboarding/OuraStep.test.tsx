...
describe("OuraStep", () => {
  let mockUpdate: Mock<(data: UserSettingsUpdate) => Promise<void>>;
  let mockNext: Mock<() => void>;
  let mockBack: Mock<() => void>;

  beforeEach(() => {
    vi.clearAllMocks();
    mockUpdate = vi.fn<(data: UserSettingsUpdate) => Promise<void>>(() =>
      Promise.resolve(),
    );
    mockNext = vi.fn<() => void>();
    mockBack = vi.fn<() => void>();
  ...
  it("shows token input when not connected", () => {
    ...
    expect(
      screen.getByPlaceholderText("Paste your Oura token"),
    ).toBeInTheDocument();
  });
  ...
  it("shows already configured message when token is set", () => {
    ...
    expect(
      screen.getByText("Oura token is already configured."),
    ).toBeInTheDocument();
  });
