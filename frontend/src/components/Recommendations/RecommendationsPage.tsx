...
export function RecommendationsPage() {
  const { data, loading, error, createExperiment, updateExperiment } = useRecommendations()
    ...
    return (
      <div className="recommendations-page" data-testid="recommendations-page">
        <div className="recommendations-gated">
          <div className="recommendations-gated-count">{data.total_days} days</div>
  ...
  const handleComplete = async () => {
    if (data.active_experiment) {
      await updateExperiment(data.active_experiment.id, { status: 'completed' })
    }
  }

  const handleAbandon = async () => {
    if (data.active_experiment) {
      await updateExperiment(data.active_experiment.id, { status: 'abandoned' })
    }
  }
    ...
    <div className="recommendations-page" data-testid="recommendations-page">
      ...
      {data.recommendations.map((rec) => (
        <RecommendationCard
          key={rec.id}
          rec={rec}
          canStartExperiment={!hasActiveExperiment && rec.suggested_experiment !== null}
      ...
      <div className="recommendations-explainer">
        ...
        {showExplainer && (
          <div className="recommendations-explainer-content">
            <p>
              These recommendations are based on patterns found in your personal data
              combined with findings from sleep research. They reflect statistical
              associations, not proven cause-and-effect relationships.
            </p>
            <p>
              Use the experiment feature to deliberately test one change at a time
              for about two weeks. This helps you see whether a specific change
              makes a measurable difference for you personally.
            </p>
          </div>
        )}
      </div>
