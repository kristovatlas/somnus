...
export function CorrelationList({ results, excludedSickDays }: CorrelationListProps) {
  if (results.length === 0) {
    return (
    ...
    <div className="analysis-card" data-testid="correlation-list">
      ...
      {excludedSickDays > 0 && (
        <p className="analysis-card-subtitle">
          {excludedSickDays} sick day{excludedSickDays !== 1 ? 's' : ''} excluded
        </p>
      )}
        ...
        {top.map((r) => (
          <div key={`${r.predictor}-${r.outcome}`} className="correlation-row">
            ...
            <div className="correlation-stats">
              <span className="correlation-r" style={{ color: rColor(r.pearson_r) }}>
                r = {r.pearson_r > 0 ? '+' : ''}{r.pearson_r.toFixed(2)}
              </span>
