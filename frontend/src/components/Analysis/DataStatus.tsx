...
function phaseLabel(unlocked: boolean, name: string, requirement: string): string {
  ...
  return (
    <div className="analysis-card" data-testid="data-status">
      ...
      <p className="analysis-card-subtitle">
        {status.total_sleep_days} sleep day{status.total_sleep_days !== 1 ? 's' : ''} recorded
      </p>

      <div className="analysis-phases">
        <span
          className="analysis-phase-pill"
          style={{ color: status.phase_a_unlocked ? 'var(--color-success)' : 'var(--color-text-muted)' }}
        >
          {phaseLabel(status.phase_a_unlocked, 'Correlations', '14+ days')}
        </span>
        <span
          className="analysis-phase-pill"
          style={{ color: status.phase_b_unlocked ? 'var(--color-success)' : 'var(--color-text-muted)' }}
        >
          {phaseLabel(status.phase_b_unlocked, 'Regression', '50+ days')}
        </span>
        <span
          className="analysis-phase-pill"
          style={{ color: status.phase_c_unlocked ? 'var(--color-success)' : 'var(--color-text-muted)' }}
        >
