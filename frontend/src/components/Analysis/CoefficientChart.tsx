...
export function CoefficientChart({ result }: CoefficientChartProps) {
  ...
  return (
    <div className="analysis-card analysis-card-wide" data-testid="coefficient-chart">
      ...
      <svg viewBox={`0 0 ${WIDTH} ${height}`} className="coefficient-svg">
        ...
        {coefs.map((c, i) => {
          ...
          const barColor = c.is_significant
            ? (c.coefficient > 0 ? 'var(--color-chart-1)' : 'var(--color-error)')
          ...
          return (
            <g key={c.predictor}>
              ...
              {c.ci_lower != null && c.ci_upper != null && (
                <>
                  <line
                    x1={scale(c.ci_lower)} y1={y}
                    x2={scale(c.ci_upper)} y2={y}
                    stroke={barColor} strokeWidth="1.5"
                  />
                  <line
                    x1={scale(c.ci_lower)} y1={y - 4}
                    x2={scale(c.ci_lower)} y2={y + 4}
                    stroke={barColor} strokeWidth="1"
                  />
                  <line
                    x1={scale(c.ci_upper)} y1={y - 4}
                    x2={scale(c.ci_upper)} y2={y + 4}
                    stroke={barColor} strokeWidth="1"
                  />
                </>
              )}
