...
**Implemented — install cooldown (ADR 014):** a ~7-day minimum release age now gates installs in both ecosystems, so a freshly-published (publish-then-yank) malicious release is not pulled: `min-release-age=7` in `frontend/.npmrc` (npm ≥ 11.10, bumped in CI) and `UV_EXCLUDE_NEWER="7 days"` on the backend `uv pip install` (CI + `make setup-backend`). The relative duration must be an env var / CLI flag — uv's `pyproject` / `uv.toml` `exclude-newer` accepts only a fixed timestamp. Security fixes inside the window stay pullable via the override path (unset the env, or `exclude-newer-package`). **Remaining:** commit a backend lockfile (`uv.lock`), add `npm audit` to CI, and pin Actions by SHA. Socket's PR alerts remain as malware detection. (Matches the CLAUDE.md/MEMORY rule: audit deps before install.)
