...
5. **Every threat resolves**: each enumerated threat carries exactly one status — **Mitigated** (with code/config citation), **Partial**, **Accepted** (residual risk Kristov signs off on), or **Open** (must become a tracked issue or a fix in the Step 9.3 audit). Nothing may remain unclassified.
...
7. **Living document**: `docs/THREAT_MODEL.md` is canonical and must never lag the code — the same currency rule as `ARCHITECTURE.md`, enforced by the per-PR "Threat model impact" statement (PLAN.md Step 9.4). Diagrams are Mermaid overlays derived from the ARCHITECTURE.md C4 views so the two documents stay maintainable together.
