...
5. **Deferred (remaining T-13):** a committed backend lockfile (`uv.lock` via `uv sync`), `npm audit` in CI, and pinning GitHub Actions by SHA — tracked in T-13, not in this change. *Status update (2026-07-09): `npm audit` in CI (lockfile-only, run before `npm ci`) and Action SHA-pinning landed in PR #65; only the backend lockfile remains deferred.*
