...
- **Done — dependency-install cooldown (T-13, ADR 014):** a ~7-day minimum release age now gates installs in both ecosystems — `min-release-age=7` in `frontend/.npmrc` (npm ≥ 11.10, bumped in CI) and `UV_EXCLUDE_NEWER="7 days"` on the backend `uv pip install` (CI + `make setup-backend`). Note the relative duration lives in env/CLI, not `pyproject` (uv accepts only a fixed timestamp there). Remaining T-13 items: commit a backend lockfile (`uv.lock`), add `npm audit` to CI, pin Actions by SHA.
