...
**COMPLETE 2026-07-09 — the gate is lifted.** 9.1+9.2: `docs/THREAT_MODEL.md` authored and human-approved (PR #42). 9.3: audit finished across PRs #43, #44, #45, #64, #65 — every entry in the 17-threat register carries an implemented mitigation or an explicit acceptance/documented residual; the sole open sub-item is the T-13 backend lockfile (`uv.lock`), deferred to a focused follow-up as a documented §7 residual. Review records live in `docs/reviews/resolved/`. 9.4: the standing per-PR rule below is wired into CLAUDE.md and the Security Review Process checklist. Normal work resumes.
