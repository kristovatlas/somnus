...
- **First remediation — dependency-install cooldown (T-13):** right after the threat-model doc merges, set a minimum release age (~7-day "cooldown") on both ecosystems and commit a backend lockfile, so a freshly-published malicious package version can't be pulled before the community detects and yanks it (the dominant publish-then-yank vector). Concretely: `min-release-age` in `frontend/.npmrc` (needs npm ≥ 11.10 — bump the CI npm off the Node 20 default), and a relative `exclude-newer` cooldown on the backend resolver (candidate: adopt `uv`, which also produces the lockfile the audit flags as missing), each with per-package overrides so security fixes aren't delayed. Native package-manager release-age gates (npm/pnpm/yarn/bun, uv) are now the primary control; Socket (already wired) and Renovate stay as belt-and-suspenders.
