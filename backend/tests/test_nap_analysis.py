...
class TestComputeNapAnalysis:
    ...
    def test_with_data(self, db: Session) -> None:
        _seed_nap_data(db)
        result = compute_nap_analysis(db)
        assert result["total_no_nap_days"] == 10
        assert result["total_nap_days"] == 10

    def test_baseline_computed(self, db: Session) -> None:
        _seed_nap_data(db)
        result = compute_nap_analysis(db)
    ...
    def test_segments_correct_bucket(self, db: Session) -> None:
        _seed_nap_data(db)
        result = compute_nap_analysis(db)
    ...
    def test_vs_no_nap_delta(self, db: Session) -> None:
        _seed_nap_data(db)
        result = compute_nap_analysis(db)
        matching = [
            s
...
class TestSegmentNaps:
    def test_empty(self) -> None:
        result = _segment_naps(
            [], {"avg_onset_latency": 10, "avg_efficiency": 0.9, "avg_total_sleep": 420}
        )
        assert result == []
    ...
    def test_bucket_boundaries(self) -> None:
        ...
        baseline: dict[str, float | None] = {
            "avg_onset_latency": 10.0,
            "avg_efficiency": 0.9,
            "avg_total_sleep": 420.0,
        }
    ...
    def test_before_1pm_bucket(self) -> None:
        nap_days = [
            {
                "start_hour": 11.0,
                "duration_minutes": 15,
    ...
    def test_after_5pm_bucket(self) -> None:
        nap_days = [
            {
                "start_hour": 17.5,
                "duration_minutes": 45,
    ...
    def test_missing_start_hour_excluded(self) -> None:
        nap_days = [
            {
                "start_hour": None,
                "duration_minutes": 25,
...
class TestTimingBuckets:
    def test_buckets_cover_all_hours(self) -> None:
    ...
    def test_duration_buckets_cover_range(self) -> None:
