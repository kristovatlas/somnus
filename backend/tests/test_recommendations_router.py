...
def _seed_data(db: Session, n: int = 60):
...
class TestGetRecommendations:
    ...
    def test_schema_validation(self, db: Session, client: TestClient):
        _seed_data(db, n=60)
        resp = client.get("/api/recommendations")
        assert resp.status_code == 200
        data = resp.json()
...
class TestExperimentCRUD:
    ...
    def test_create_conflict_409(self, client: TestClient):
    ...
    def test_experiment_with_computed_metrics(self, db: Session, client: TestClient):
...
class TestDashboardTopRecommendations:
    def test_present_when_data(self, db: Session, client: TestClient):
        _seed_data(db, n=60)
    ...
    def test_empty_when_insufficient(self, client: TestClient):
        resp = client.get("/api/dashboard")
        assert resp.status_code == 200
        data = resp.json()
