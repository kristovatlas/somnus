...
def _seed_data(db: Session, n: int = 20) -> None:
...
class TestAnalysisStatus:
    ...
    def test_variable_fields(self, db: Session, client: TestClient) -> None:
        _seed_data(db, n=20)
        resp = client.get("/api/analysis/status")
        data = resp.json()
...
class TestAnalysisCorrelations:
    ...
    def test_r_values_bounded(self, db: Session, client: TestClient) -> None:
        _seed_data(db, n=30)
        resp = client.get("/api/analysis/correlations")
        data = resp.json()
    ...
    def test_correlation_fields(self, db: Session, client: TestClient) -> None:
        _seed_data(db, n=20)
        resp = client.get("/api/analysis/correlations")
        data = resp.json()
        for r in data["results"]:
...
class TestAnalysisRegression:
    ...
    def test_insufficient_data(self, db: Session, client: TestClient) -> None:
        _seed_data(db, n=10)
        resp = client.get("/api/analysis/regression")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_with_sufficient_data(self, db: Session, client: TestClient) -> None:
        _seed_data(db, n=60)
        resp = client.get("/api/analysis/regression")
        assert resp.status_code == 200
        data = resp.json()
